...
function fc_safeCloseWS() {
  if (window.fc_ws) {
    try { window.fc_ws.close(); } catch (_) {}
  }
  window.fc_ws = null;
  window.fc_wsActive = false;
}
...
// DEFAULT ACTIONS
  ...
  fc_headers["Content-Type"] = "application/x-www-form-urlencoded";
    ...
    if (window.fc_last_user_public && typeof fc_getUpdates_fnc === "function" && !window.fc_wsActive) {
      setTimeout(() => {
        fc_getUpdates_fnc(
          "initialize",
          window.fc_cfg,
...
function fc_getUpdates_fnc(
  ...
  if (window.fc_priorityLock || window.fc_wsActive || !fc_user_id) return;
  ...
  const fc_openWS_fnc = () => {
    ...
    socket.onopen = () => {
      ...
      socket.send(JSON.stringify({
        user_id: fc_user_id // replace with your logic
      }));
    ...
    socket.onmessage = (event) => {
      try {
        ...
        // Handle update event (replace push_count logic)
    ...
    // Error or close → reconnect with backoff
    const scheduleReconnect = () => {
      stopHeartbeat();
      window.fc_wsActive = false;
      setTimeout(fc_openWS_fnc, window.fc_streamBackoff || 1000);
      fc_increaseBackoff();
    };
